...
import { AvantisSDK } from './src/AvantisSDK';
import { PositionSide, OrderType } from './src/types';
...
async function testPrepareApprove() {
  ...
  try {
    const sdk = new AvantisSDK('base-sepolia'); // Use testnet
...
async function testPrepareOpenPosition() {
  ...
  try {
    ...
    // Check that data is encoded function call (openTrade selector: 0x...)
