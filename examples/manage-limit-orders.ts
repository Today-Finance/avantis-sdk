...
async function manageOrdersExample() {
  ...
  try {
    ...
    console.log(`Status: ${marketOrder.success ? '✅ Executed' : '❌ Failed'}\n`);
    ...
    // 3. STOP ORDER - Sell if price drops (stop loss order)
    console.log('3. Stop Order (Stop Loss)');
    console.log('-------------------------');
    ...
    console.log(`Stop Order TX: ${stopOrder.transactionHash}`);
    ...
    console.log('2. Limit Order: PENDING - Will buy ETH at', formatPrice(limitPrice));
    console.log('3. Stop Order: PENDING - Will short ETH at', formatPrice(stopPrice));
    ...
    console.log(`Crypto (${cryptoPairs.length} pairs):`, cryptoPairs.slice(0, 5).join(', '), '...');
    ...
    console.log(`Commodities (${commodityPairs.length} pairs):`, commodityPairs.join(', '));
