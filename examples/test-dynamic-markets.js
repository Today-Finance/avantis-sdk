...
async function testDynamicMarkets() {
  ...
  try {
    ...
    markets.slice(0, 5).forEach((market, index) => {
      console.log(`\n${index + 1}. Market Index: ${market.pairIndex}`);
      ...
      console.log(`   Min Leverage: ${market.minLeverage}x`);
    });
    ...
    const withPrices = marketsWithPrices.filter(m => m.currentPrice !== undefined);
    const withoutPrices = marketsWithPrices.filter(m => m.currentPrice === undefined);
    ...
    withPrices.slice(0, 5).forEach((market, index) => {
      ...
      if (price) {
        ...
        const priceValue = expo < 0
          ? (parseFloat(price.price) / Math.pow(10, Math.abs(expo))).toFixed(Math.abs(expo))
          : parseFloat(price.price) * Math.pow(10, expo);
    ...
    if (withoutPrices.length > 0) {
      ...
      withoutPrices.forEach(market => {
        console.log(`   - ${market.name || 'Unknown'} (Index: ${market.pairIndex}, Feed: ${market.pythFeedId})`);
      });
    }
    ...
    if (pairsCount > 0) {
      ...
      console.log('\nFirst pair raw data (sample):');
      console.log(`  Pair Index: ${pairData.pairIndex}`);
      console.log(`  Feed ID: ${pairData.feed.feedId}`);
      console.log(`  Min Leverage: ${pairData.leverages.minLeverage}`);
      console.log(`  Max Leverage: ${pairData.leverages.maxLeverage}`);
      console.log(`  Spread: ${pairData.spreadP}`);
    }
